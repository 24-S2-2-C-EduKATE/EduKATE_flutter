...
import 'package:flutter_application_1/picture_block/models/block_shape.dart';
import 'package:flutter_application_1/picture_block/models/block_with_image.dart';
import 'package:audioplayers/audioplayers.dart'; // 引入声音播放库
...
class _CommandManagerState extends State<CommandManager> {
  ...
  String selectedCategory = 'Events';
  String? selectedBlockId; // 当前被选中的 block id
  String? hoveringBlockId; // 当前悬停的 block id

  // 简单播放声音，每次点击新建一个 AudioPlayer
  Future<void> playSimpleSound() async {
    try {
      final AudioPlayer audioPlayer = AudioPlayer(); // 每次点击新建播放器
      await audioPlayer.play(AssetSource('assets/sounds/click.mp3')); // 播放 asset 声音
    } catch (e) {
      print('播放声音失败: $e');
    }
  }
